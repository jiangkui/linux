...
#-----------------数组--------------------
declare -A dialDatePrice #server 地址
...
function printPrice(){
    index=0;
    sumPrice=0;
    for item in ${ZhongJiangQingKuang}
    do
        ((index++))
        ((priceIdx=index%3))
...
select * from (
    select
        ...
        dial_prize_id
    from one.dial_prize_record where date(create_time) = '${dateParam}' group by dial_prize_id
) res  order by res.num desc;"`
echo ""
echo "=================================="
echo "各种奖项今天中奖情况："
#echo "${ZhongJiangQingKuang}"
printPrice ${ZhongJiangQingKuang}
