...
![3-1进程描述符及任务队列](./img/3-1进程描述符及任务队列.png)
...
![3-2进程描述符和内核栈](./img/3-2进程描述符和内核栈.png)
...
## 第4章 进程调度
>目标: 通过合理的调度, 最大限度的发挥系统资源。
>原则: 只要有可以执行的进程, 那么就总会有进程在执行。(最大限度的利用处理器时间)
>本章讲解调度程序的设计基的基础和完全公平调度程序如何运用、如何设计、如何实现以及它相关的系统调用。也会讲解 O(1) 调度程序, 毕竟是经典 Unix 调度程序模型的实现方式。

### 4.1 多任务
- 多任务系统分类:
    - 抢占式: Linux 使用此种模式。
    - 非抢占式: 需要线程自己主动停止运行, 否则它会一直执行。现在已经没有系统采用这种方式了。
- 时间片: 进程能够运行的时间。有效管理时间片能够使调度程序从系统全局的角度做出调度决定。但 Linux 没采取时间片来达到公平调度。

### 4.2 Linux 的进程调度
- 原始调度: 2.5 版本之前
    - 优点: 易理解
    - 缺点: 多进程和多处理器情况下, 难以胜任。
- O(1) 调度: 2.6.23 版本之前使用此方式调度。
    - 优点: 在数十个多处理器环境下性能不错。
    - 缺点: 对响应时间敏感的程序不足, 如用户交互程序。
- 完全公平调度算法(CFS): 前身是 反转楼梯最后期限调度算法(Rotating Staircase Deadline scheduler)(RSDL)
    - 优点: 解决了 O(1) 的不足

### 4.3 策略
策略决定何时让什么程序运行, 调度器的策略决定系统的整体印象, 还负责优化使用处理器的时间。所以策略很重要。

#### 4.3.1 I/O 消耗型和处理器消耗型的进程
- 进程分两种类型:
    - I/O 消耗型: 进程的大部分时间用来提交 I/O 请求或等待 I/O 请求。这样的进程经常处于可运行状态,但通常都是运行一小会, 大部分时间是在等待。
    - 处理器消耗型: 把时间大多用在执行代码上。除非被抢占, 否则他们通常都一直不停的运行。
        - 调度策略: 往往是降低调度频率, 延长运行时间。

- 调度策略通常需要在两个矛盾的目标中间寻找平衡:
    - 响应时间短: 进程响应迅速
    - 高吞吐率: 最大系统利用率

#### 4.3.2 进程优先级
- Linux 采用了两种不同的优先级范围:
    - nice 值: 从 -20~19, 值越低, 优先级越高。`ps -el 命令查看进程列表, NI 列就是进程对应的 nice 值`
    - 实时优先级: 可配置, 默认 0~99, 值越高, 优先级越高。

#### 4.3.3 时间片
- 时间片: 进程在被抢占前所能持续运行的时间。
- 调度策略必须规定默认的时间片, 但有矛盾:
    - 时间片太短: 会增大进程切换带来的处理器耗时。
    - 时间片过长: 交互相应就会表现欠佳。
    - 矛盾: I/O 消耗型不需要长的时间片, 处理器消耗型希望越长越好。
- Linux 是把时间片变成处理器使用比了, 还会受到 nice 值的影响。低 nice 值将获得更多的处理器使用比。

#### 4.3.4 调度策略的活动
- 给了个例子: 假设一个系统有两个进程, 一个文本编辑程序(I/O消耗型)和一个视频编码程序(处理器消耗型), 如何调度?
    - 多数操作系统: 给文本编辑器更高的优先级和更多的时间片。但会有浪费, 等待I/O嘛。
    - Linux 实现: 文本编辑器需要时就抢占, 视频处理程序占用剩下的时间。(假设nice值相等), CFS注意到文本编辑器使用比率是50%, 但实际时间短得多, 所以此时会让文本编译器抢占。

### 4.4 Linux 调度算法
`前面的内容讨论进程调度原理, 现在讨论 Linux 特色的进程调度程序`

#### 4.4.1 调度器类
- Linux 调度器是以模块化方式提供的, 目的是: 运行不同类型的进程可以有针对性的选择调度算法。
- 完全公平调度(CFS): 是一个真的普通进程的调度类, 在 `kernel/sched_fair.c` 中。

#### 4.4.2 Unix 系统中的进程调度
传统 Unix 系统调度的问题: 通过进程优先级和时间片来进行调度。
- nice 值对应绝对时间片: 低优先级获得的时间少, 高优先级获得的时间多, 但低优先级一般都是密集计算型的, 希望运行时间长, 调度次数少。有冲突。
- nice 值对应相对时间片: 两个进程, 假设nice 值是0和1, 对应的时间片是 100和95, 但如果nice 如果为 18和19, 则对应的时间片为 10ms和5ms, 前者是后者两倍!
- 如果执行nice值到时间片的映射, 则需要一个绝对时间片, 而且绝对时间片必须能在内核的测试范围内。
    - 意味着: 时间片必须是节拍器的整数倍。耦合了。时间片会随着定时器节拍改变。(`这是引入 CFS 的唯一原因!`)
- 如何处理交互任务的问题。
- 综上所述:
    - 传统调度本质问题: 时间片不公平。
    - CFS解决方案: 不用时间片, 改为处理器使用比重。

#### 4.4.3 公平调度(CFS)
- 概念: 进程调度的效果应如同系统具备一个理想中的完美多任务处理器。在这种系统中, 每个进程将能获得 1/n 的处理器时间。
- 做法: 运行每个进程运行一段时间、循环轮转、 选择运行最小的进程作为下一个运行进程, 不再采用分配给每个进程时间片的做法了。
- 保底: 按权重均分 CPU 时间片, 进程越多, 运行时间越短, 最小底线值是1ms, 不然进程太多, 就没法搞了。

### 4.5 Linux 调度的实现
相关代码位于 `kernel/sched_fair.c`, 主要关注四部分:
- 时间记账:
- 进程选择:
- 调度器入口:
- 睡眠和唤醒:
...
#### 4.5.1 时间记账
所有的调度器都必须对进程运行时间做记账。
- 旧Unix: 每次系统时间节拍发生时, 时间片减少一个节拍周期, 时间片为0时, 会被另外一个不为0的进程抢占。
- CFS: 系统定时器周期性调用, 准确测量程序的运行时间。

##### 1.调度器实体结构
CFS 没时间片的概念, 但也得维护每个进程运行的时间记账。这样才能确保每个进程只在公平分配给他的处理器时间内运行。
CFS 使用调度器实体结构(定义在 `<linux/sched.h>`的 struct_sched_entity 中)来追踪运行记账。`task_stuct->se` 就是这个实体结构。sched_entity。`、

##### 2.虚拟实时
vruntime 变量存放了虚拟运行时间, 单位是 ns,vruntime 和定时器节拍不在相关。
- `kernel/sched_fair.c` update_curr() 函数实现了记账功能, _update_curr() 对时间进行加权运算。
- update_curr() 是有系统定时器周期性调用的, 所以可以准确测量进程的运行时间, 而且可以知道下一个运行的是谁。

#### 4.5.2 进程选择
**CFS调度算法的核心: 选择具有最小 vruntime 的任务。CFS 使用红黑树实现, 最左侧叶子节点即: 最小 vruntime 值的进程, 此节点会被缓存, 不需要遍历树。cfs_rq->rb_leftmost**

##### 1.挑选下一个任务
- 选择 vruntime 最小的任务。即: 红黑树选择最左侧的叶子节点, 此节点有一个变量指向他, 不需要每次都遍历树。当没有可运行的任务, CFS 会选择 idle 任务运行。
- 代码: `kernel/sched_fair.c _pick_next_entity()`
...
- 流程:
    1. 更新运行时间和其他统计数据
    2. 调用 _enqueue_entity() 进行繁重的插入操作。把进程插入到红黑树里面。
    3. 插入时, 维护最左侧节点缓存。
    4. 平衡红黑树
- 代码: `kernel/sched_fair.c enqueue_entity()`
...
- 流程:
    ...
    2. 从红黑树中删掉进程
    3. 维护最左侧节点缓存
- 代码:`kernel/sched_fair.c dequeue_entity()`

#### 4.5.3 调度器入口
- 代码:`kernel/sched.c schedule() 中, 该函数会调用 pick_next_task()`
- 流程: 选择哪个进程可以运行, 何时投入运行。`由 pick_next_task() 实现此功能`
    1. 优化: 如果所有任务都在 CFS 里面(所有可运行进程数等于CFS对应的可运行进程数), 则直接获取红黑树最左侧节点的进程。即: `return cfs.pick_next_task();`
    2. 从高到底检查调度类
    3. 从最高优先级的调度类中, 选择最高优先级的进程。
- 功能: 如果是进程主动调用该方法, 意味着进程主动让出CPU。原因可能是该进程需要等待资源而暂时停止运行, 就主动让出CPU了。

#### 4.5.4 睡眠和唤醒
- 睡眠: `休眠(被阻塞)的进程处于不可执行的状态, 需要从可执行红黑树中移出。`
- 流程:
    1. 进程把自己标记成休眠状态
    2. 从红黑树中移出, 放入等待队列。
    3. 调用 schedule() 选择和执行一个其他进程。
- 唤醒: 流程与睡眠相反, 进程被设置为可执行状态, 从等待队列中移到可执行红黑树中。

##### 1.等待队列
- 数据结构: 简单的链表结构。
- 重点: 为了避免产生竞争条件, 休眠和唤醒的实现不能有纰漏。
- 流程: 进程通过以下几个步骤把自己加入到一个等待队列中
    1. 调用宏 define_wait() 创建一个等待队列的项。
    2. 调用 add_wait_queue() 把自己加入到等待队列中。条件满足时会唤醒。
    3. 调用 prepare_to_wait() 方法将进程状态变更为 TASK_INTERRUPTIBLE 或 TASK_UNINTERRUPTIBLE。
    4. 如果状态被设置为 TASK_INTERRUPTIBLE, 则信号唤醒进程。
    5. 当进程被唤醒的时候, 他会再次检查条件是否为真。如果条件ok, 则退出循环。否则它再次调用 schedule() 主动让出CPU。
    6. 条件满足时, 进程将自己设置为 TASK_RUNNING 并调用 finish_wait() 放他把自己移出等待队列。
- 实现: `fs/notify/inotify/inotify_user.c 中 inotify_read() 上述流程的一个具体实现。`

##### 2.唤醒
- 函数: 通过 wake_up(), 他会唤醒指定的等待队列上的所有进程。
- 流程:
    1. wake_up() 调用 try_to_wake_up(), 该函数负责将进程设置为 TASK_RUNNING 状态。
    2. 调用 enqueue_task() 将此进程放入红黑树中, 如果被唤醒的进程优先级比当前正在执行的进程的优先级高, 还需要设置 need_resched标志。(会导致 schedule() 被调用, 此时会发生抢占)

![4-1休眠和唤醒](./img/4-1休眠和唤醒.png)

### 4.6 抢占和上下文切换
- 上下文切换: 一个进程切换到另一个进程。
- 代码: `kernel/sched.c context_switch()` 主要有两个功能:
    - 内存映射切换: 调用 `asm/mmu_context.h switch_mm()`, 把虚拟内存从上一个进程映射切换到新进程中。
    - 处理器切换: 调用 `asm/system.h switch_to()`, 把上一个进程的处理器状态切换到新进程的处理器状态。包括: 保存、恢复栈信息和寄存器信息。还有其他任何与体系结构相关的状态信息。
- 内核需要知道在什么时候调用 schedule():
    - 内核提供了一个 need_resched 标志来表明是否需要重新执行一次调度。
    - 进程被抢占、优先级高的进程进入可执行状态 都会设置该状态。
    - 每个进程都有一个 need_resched 标志, 原因在于: 访问进程描述符内的数值要比访问全局变量快。(current 宏很快, 并且描述符通常都在高速缓存里)

#### 4.6.1 用户抢占
- 内核无论是在中断处理程序还是在系统调用后返回, 都会检查 need_resched 标志。如果被设置, 就会调用 schedule()。
- 简而言之, 抢占发生在:
    - 从系统调用返回用户空间时
    - 从中断处理程序返回用户空间时

#### 4.6.2 内核抢占
- 2.6 之后内核支持抢占, 前提是: 不持有锁。
    - 使用 thread_info.preempt_count 来计数, 使用一次锁, 计数加1. 数值为0的时候可以进行抢占。
- 内核抢占时机:
    - 中断处理程序正在执行, 且返回内核空间之前
    - 内核代码再一次具有可抢占性的时候。
    - 内核中的任务显式的调用 schedule();
    - 内核中的任务阻塞(会导致调用 schedule())

### 4.7 实时调度策略
- Linux 提供三种度策略:
    - 实时调度策略(两种):
        - SCHED_FIFO: **不使用时间片**, 优先级高于 SCHED_RR, 有可执行的进程, 就会一直执行。直到自己阻塞或显式的释放处理器。
        - SCHED_RR: **使用时间片**, 时间片耗尽之后就不能在继续执行了。
        - 优先级范围: 0 ~ MAX_RT_PRIO-1, MAX_RT_PRIO 一般为100
    - 非实时调度策略:
        - SCHED_NORMAL:
    - 注意: 这些策略不被 CFS 管理, 由其他调度器管理。

### 4.8 与调度相关的系统调用
提供了系统调用方法, 可修改进程的优先级、调度策略、处理器绑定, 还提供显示的将处理器交个其他进程的机制。
![4-2与调度相关的系统调用](./img/4-2与调度相关的系统调用.png)

#### 4.8.1 与调度策略和优先级相关的系统调用
- sched_setscheduler 和 sched_getsheduler(): 设置和获取进程的调度策略和实时优先级。
    - 功能: 与其他系统调用类似, 实现是由许多参数检查、初始化和清理构成。
    - 最重要的工作: 读取和改写进程 tast_struct 的 policy 和 rt_priority 的值。
- sched_setparam() 和 sched_getparam(): 设置和获取进程的实施优先级。
- 对于普通进程, nice() 可以给进程的静态优先级增加一个给定的量, 只有超级用户才能在调用时使用负值。

#### 4.8.2 与处理器绑定有关的系统调用。
- Linux 调度程序提供强制的处理器绑定机制。
- 强制机制的亲和性保存在进程 task_struct 的 cpus_allowed 这个掩码位标志中。掩码的每一位对应一个系统可用的处理器。默认情况下所有位都被设置, 即进程可以在所有可用的处理器上执行。
- sched_setaffinity() 和 sched_getaffinity(): 设置和获取进程的掩码位。
- 进程与 CPU 的绑定和变更:
    - 进程第一次创建的时候, 继承父进程的相关掩码。
    - 处理器绑定关系变更时, 内核会采用 "移植线程" 把任务推到合法的处理器上。

#### 4.8.3 放弃处理器时间
- 如何放弃: sched_yield() 系统调用实现。
    - 它将进程从 **活动队列** 移到 **过期队列** 实现。
    - 实时进程不会过期。它们只会被移动到优先级队列的最后面, 不会放到过期队列中。

### 4.9 小结
- 进程调度程序是内核重要的组成部分。
- 满足进程调度的各种需求绝不是轻而易举的: 很难找到 "一刀切" 的算法, 即适合众多的可运行进程, 又具有可伸缩性, 还能在调度周期和吞吐率之间求得平衡, 同时还满足各种负载的需求。
- 前面章节介绍进程管理的内容。本证介绍了: 进程调度所遵循的基本原理、具体实现、调度算法以及目前 Linux 内核使用的接口。
