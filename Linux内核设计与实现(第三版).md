...
#### 4.4.3 公平调度
- CFS 的概念: 进程调度的效果应如同系统具备一个理想中的完美多任务处理器。在这种系统中, 每个进程将能获得 1/n 的处理器时间。
- CFS 的做法: 运行每个进程运行一段时间、循环轮转、 选择运行最小的进程作为下一个运行进程, 不再采用分配给每个进程时间片的做法了。
- 按权重均分 CPU 时间片, 进程越多, 运行时间越短, 最小底线值是1ms, 不然进程太多, 就没法搞了。
...
##### 2.虚拟实时
vruntime 变量存放了虚拟运行时间, 单位是 ns,vruntime 和定时器节拍不在相关。 定义在 `kernel/sched_fair.c 重点  update_curr()函数实现了记账功能。`
